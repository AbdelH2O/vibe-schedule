...
// Inner content component that remounts when dialog opens, resetting all state
function SessionSetupDialogContent({
  onOpenChange,
}: {
  onOpenChange: (open: boolean) => void;
}) {
  ...
  // Initialize selection with all contexts - this resets each time component mounts
  const [selectedContextIds, setSelectedContextIds] = useState<Set<string>>(
    () => new Set(state.contexts.map(c => c.id))
  );

  // Calculate allocations when duration or selection changes
  const allocationResult = useMemo(() => {
    if (sessionMinutes === null || sessionMinutes <= 0) {
      return null;
    }

    const selectedContexts = state.contexts
      .filter(ctx => selectedContextIds.has(ctx.id))
      .map((ctx) => ({
        id: ctx.id,
  ...
  }, [sessionMinutes, state.contexts, selectedContextIds]);
  ...
  const handleSavePreset = useCallback(() => {
    ...
    setPresetName('');
  }, [allocationResult, sessionMinutes, presetName, addPreset, state.contexts, selectedContextIds]);
  ...
  // Can start if we have a valid result, duration, and at least one context selected
  const canStart =
...
export function SessionSetupDialog({ open, onOpenChange }: SessionSetupDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {open && <SessionSetupDialogContent onOpenChange={onOpenChange} />}
    </Dialog>
  );
}
